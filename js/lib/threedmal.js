...
var ThreeDmal = {
    ...
    Camera:function (viewportWidth, viewportHeight) {
        var that = this;
        this.position = new ThreeDmalMath.Vector3(1, 1, 1);
        this.target = new ThreeDmalMath.Vector3(0, 0, 0);
        this.up = new ThreeDmalMath.Vector3(0, 1, 0);
        const aspectRatio = viewportWidth / viewportHeight;
        this.projectionMatrix = ThreeDmalMath.createPerspectiveFieldOfViewMatrix(Math.PI / 4, aspectRatio, 1, 1000);
        this.viewProjectionMatrix = null;

        /**
         * @param {ThreeDmalMath.Vector3} value
         */
        ...
        this._refreshViewProjectionMatrices = function () {
            const viewMatrix = ThreeDmalMath.createLookAtMatrix(that.position, that.target, that.up);
            that.viewProjectionMatrix = ThreeDmalMath.multiplyMatrixMatrix(viewMatrix, that.projectionMatrix);
        };
    ...
    Model:function () {
        ...
        this.position = new ThreeDmalMath.Vector3(0, 0, 0);
        this.rotation = new ThreeDmalMath.Vector3(0, 0, 0);
        this.scale = new ThreeDmalMath.Vector3(1, 1, 1);
        ...
        this.addVertex = function (x, y, z) {
            ...
            that.vertices[position] = new ThreeDmalMath.Vector4(x, y, z, 1);
        };
        ...
        this._refreshWorldMatrix = function () {
            var rotationMatrix = ThreeDmalMath.createRotationMatrix(that.rotation.y, that.rotation.x, that.rotation.z),
                scaleMatrix = ThreeDmalMath.createScaleMatrix(that.scale),
        ...
        this.draw = function () {
            ...
            for (var i = 0; i < models.length; i++) {
                ...
                for (var k = 0; k < trianglesCount; k++) {
                    ...
                    that.context.moveTo(vertex1.x, vertex1.y);
                    that.context.lineTo(vertex2.x, vertex2.y);
                    that.context.lineTo(vertex3.x, vertex3.y);
                    that.context.lineTo(vertex1.x, vertex1.y);
                }
...
var ThreeDmalMath = {
     ...
     *   12 13 14 15
     * @constructor
     * @return {ThreeDmalMath.Matrix4x4}
     */
    Matrix4x4:function () {
        this[0] = 0;
        this[1] = 0;
    ...
    Vector3:function (x, y, z) {
        ...
        this.normalize = function () {
            var num = Math.sqrt(ThreeDmalMath.dotProductVector3(that, that));
            that.x /= num;
            that.y /= num;
            that.z /= num;
        };
    ...
    /**
     * @param {ThreeDmalMath.Vector3} position
     * @param {ThreeDmalMath.Vector3} target
     * @param {ThreeDmalMath.Vector3} up
     * @return {ThreeDmalMath.Matrix4x4}
     */
    createLookAtMatrix:function (position, target, up) {
        var targetPosition = ThreeDmalMath.substractVector3(position, target);
        targetPosition.normalize();
        var upTargetPositionCrossProduct = ThreeDmalMath.crossProductVector3(up, targetPosition);
        upTargetPositionCrossProduct.normalize();
        var actualUp = ThreeDmalMath.crossProductVector3(targetPosition, upTargetPositionCrossProduct);
        var result = new ThreeDmalMath.Matrix4x4();
        ...
        result[12] = -ThreeDmalMath.dotProductVector3(upTargetPositionCrossProduct, position);
        result[13] = -ThreeDmalMath.dotProductVector3(actualUp, position);
        result[14] = -ThreeDmalMath.dotProductVector3(targetPosition, position);
        result[15] = 1;
    ...
    /**
     * @param {ThreeDmalMath.Vector3} vector1
     * @param {ThreeDmalMath.Vector3} vector2
     * @return {ThreeDmalMath.Vector3}
     */
    crossProductVector3:function (vector1, vector2) {
        ...
        return new ThreeDmalMath.Vector3(x, y, z);
    },
    ...
    /**
     * @param {ThreeDmalMath.Matrix4x4} m1
     * @param {ThreeDmalMath.Matrix4x4} m2
     * @return {ThreeDmalMath.Matrix4x4}
     */
    ...
    /**
     * @param {ThreeDmalMath.Vector4} vector
     * @param {ThreeDmalMath.Matrix4x4} matrix
     * @return {ThreeDmalMath.Vector4}
     */
    multiplyVector4Matrix:function (vector, matrix) {
        ...
        return new ThreeDmalMath.Vector4(x, y, z, w);
    },

    /**
     * @param {ThreeDmalMath.Vector4} point
     * @param {ThreeDmalMath.Matrix4x4} worldViewProjectionMatrix
    ...
    project:function (point, worldViewProjectionMatrix, screenWidth, screenHeight) {
        ...
        return new ThreeDmalMath.Vector2(
    ...
    substractVector3:function (vector1, vector2) {
        return new ThreeDmalMath.Vector3(
            vector1.x - vector2.x,
            vector1.y - vector2.y,
            vector1.z - vector2.z);
    },
